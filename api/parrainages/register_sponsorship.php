...
if (!$periode) {
    echo json_encode([
        "success" => false,
        "message" => "Aucune periode de parrainage n'est definie."
    ]);
    exit();
}
...
if ($currentDate < $periode['date_debut']) {
    echo json_encode([
        "success" => false,
        "message" => "La periode de parrainage n'a pas encore commence. Impossible de parrainer."
    ]);
    exit();
}
if ($currentDate > $periode['date_fin']) {
    echo json_encode([
        "success" => false,
        "message" => "La periode de parrainage est terminee. Impossible de parrainer."
    ]);
    exit();
}
...
if (!isset($data->numero_electeur, $data->numero_cin, $data->code_auth)) {
    echo json_encode([
        "success" => false,
        "message" => "Veuillez saisir votre numero d'electeur, votre numero de CIN et votre code d'authentification."
    ]);
    exit();
}
...
if (!isset($data->sponsorship_code)) {
    ...
    if ($stmt->fetch()) {
        echo json_encode([
            "success" => false,
            "message" => "Vous avez deje parraine un candidat."
        ]);
        exit();
    }
    ...
    if ($insert) {
        ...
        echo json_encode([
            "success" => true,
            "message" => "Code de validation envoye par email. Veuillez verifier votre boite mail."
        ]);
    } else {
        echo json_encode([
            "success" => false,
    ...
    if ($update) {
        $confirmationMsg = "Votre parrainage a ete confirme. Vous avez choisi le candidat {$sponsorship['candidate_nom']} du parti {$sponsorship['parti_politique']}.";
        sendEmail($parrain["email"], "Parrainage confirme", $confirmationMsg);
        echo json_encode([
            "success" => true,
